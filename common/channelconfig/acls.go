...
func newAPIsProvider(acls map[string]*pb.APIResource) *aclsProvider {
	...
	for key, acl := range acls {
		...
		if acl.GetPolicyRef()[0] != '/' {
			aclPolicyRefs[key] = "/" + ChannelGroupKey + "/" + ApplicationGroupKey + "/" + acl.GetPolicyRef()
		} else {
			aclPolicyRefs[key] = acl.GetPolicyRef()
		}
	}
