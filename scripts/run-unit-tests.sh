...
run_tests() {
  ...
  time {
    ...
    if [ "${#serial[@]}" -ne 0 ]; then
      gotestsum --format standard-quiet --junitfile unit-tests.xml -- -coverprofile=cover.out "${flags[@]}" -tags "$GO_TAGS" "${serial[@]}" -short -p 1 -timeout=20m
    fi

    local -a parallel
    while IFS= read -r pkg; do parallel+=("$pkg"); done < <(parallel_test_packages "$@")
    if [ "${#parallel[@]}" -ne 0 ]; then
      gotestsum --format standard-quiet --junitfile unit-tests.xml -- -coverprofile=cover.out "${flags[@]}" "${race_flags[@]}" -tags "$GO_TAGS" "${parallel[@]}"
    fi
  }
}
