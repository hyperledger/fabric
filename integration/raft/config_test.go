...
var _ = Describe("EndToEnd reconfiguration and onboarding", func() {
	...
	Describe("Invalid Raft config metadata", func() {
		It("refuses to start orderer or rejects config update", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			err = os.WriteFile(network.OutputBlockPath(network.SystemChannel.Name), data, 0o644)
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(err).NotTo(HaveOccurred())
			err = os.WriteFile(network.CreateChannelTxPath(channel), data, 0o644)
			Expect(err).NotTo(HaveOccurred())
	...
	When("an orderer node is joined", func() {
		It("isn't influenced by outdated orderers", func() {
			...
			genesisBootBlock, err := os.ReadFile(filepath.Join(testDir, "systemchannel_block.pb"))
			Expect(err).NotTo(HaveOccurred())
			restoreBootBlock := func() {
				err = os.WriteFile(filepath.Join(testDir, "systemchannel_block.pb"), genesisBootBlock, 0o644)
				Expect(err).NotTo(HaveOccurred())
			}
