...
func buildCert(caBytes []byte, orgCAPath string, csr *x509.CertificateRequest, serialNumber *big.Int, pubKey *ecdsa.PublicKey) []byte {
	...
	pk, ok := caCert.PublicKey.(*ecdsa.PublicKey)
	Expect(ok).To(BeTrue())

	keyBytes, err := ioutil.ReadFile(filepath.Join(orgCAPath, fmt.Sprintf("%x_sk", computeSKI(pk))))
	Expect(err).NotTo(HaveOccurred())
...
func updateMSPFolder(path, certName string, cert []byte) {
	...
	matches, err := filepath.Glob(filepath.Join(path, "keystore", "*_sk"))
	Expect(err).NotTo(HaveOccurred())
...
func computeSKI(publicKey *ecdsa.PublicKey) []byte {
