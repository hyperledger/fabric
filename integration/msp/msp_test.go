...
var _ = Describe("MSP identity test on a network with mutual TLS required", func() {
	...
	It("invokes chaincode on a peer that does not have a valid endorser identity", func() {
		...
		sess, err := network.PeerCliUserSession(org2Peer0, "User1", commands.ChaincodeQuery{
		...
		sess, err = network.PeerCliUserSession(org2Peer0, "User1", commands.ChaincodeInvoke{
			ChannelID: "testchannel",
		...
		sess, err = network.PeerCliUserSession(org1Peer0, "User1", commands.ChaincodeInvoke{
			ChannelID: "testchannel",
			Orderer:   network.OrdererAddress(orderer, nwo.ListenPort),
			Name:      "mycc",
			Ctor:      `{"Args":["invoke","a","b","10"]}`,
		...
		By("reverifying the channel was not affected by the unauthorized endorsement")
		sess, err = network.PeerCliUserSession(org1Peer0, "User1", commands.ChaincodeQuery{
			ChannelID: "testchannel",
			Name:      "mycc",
			Ctor:      `{"Args":["query","a"]}`,
		})
...
func RunQueryInvokeQuery(n *nwo.Network, orderer *nwo.Orderer, peer *nwo.Peer, initialQueryResult int) {
	...
	sess, err = n.PeerCliUserSession(peer, "User1", commands.ChaincodeInvoke{
		ChannelID: "testchannel",
