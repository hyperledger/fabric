...
var _ = Describe("EndToEnd Smart BFT configuration test", func() {
	...
	Describe("smartbft network", func() {
		It("smartbft multiple nodes stop start all nodes", func() {
			...
			sess, err := network.PeerCliUserSession(peer, "User1", commands.ChaincodeQuery{
				ChannelID: channel,
				Name:      "mycc",
				Ctor:      `{"Args":["query","a"]}`,
			})
		...
		It("smartbft multiple nodes view change", func() {
			...
			sess, err := network.PeerCliUserSession(peer, "User1", commands.ChaincodeInvoke{
				ChannelID: channel,
				Orderer:   endpoints,
				Name:      "mycc",
		...
		It("smartbft the leader froze, waiting for an answer", func() {
			...
			sess, err := network.PeerCliAdminSession(peers[0], commands.ChaincodeApproveForMyOrg{
				ChannelID:           channel,
