...
func PackageChaincode(n *Network, chaincode Chaincode, peer *Peer) {
	sess, err := n.PeerCliAdminSession(peer, commands.ChaincodePackage{
		Path:       chaincode.Path,
		Lang:       chaincode.Lang,
...
func CheckPackageID(n *Network, packageFile string, packageID string, peer *Peer) {
	sess, err := n.PeerCliAdminSession(peer, commands.ChaincodeCalculatePackageID{
...
func InstallChaincode(n *Network, chaincode Chaincode, peers ...*Peer) {
	...
	for _, p := range peers {
		sess, err := n.PeerCliAdminSession(p, commands.ChaincodeInstall{
...
func CommitChaincode(n *Network, channel string, orderer *Orderer, chaincode Chaincode, peer *Peer, checkPeers ...*Peer) {
	...
	sess, err := n.PeerCliAdminSession(peer, commands.ChaincodeCommit{
		ChannelID:           channel,
		Orderer:             n.OrdererAddress(orderer, ListenPort),
		Name:                chaincode.Name,
		Version:             chaincode.Version,
...
func QueryInstalled(n *Network, peer *Peer) func() []lifecycle.QueryInstalledChaincodesResult_InstalledChaincode {
	return func() []lifecycle.QueryInstalledChaincodesResult_InstalledChaincode {
		sess, err := n.PeerCliAdminSession(peer, commands.ChaincodeQueryInstalled{
			ClientAuth: n.ClientAuthRequired,
		})
		Expect(err).NotTo(HaveOccurred())
		Eventually(sess, n.EventuallyTimeout).Should(gexec.Exit(0))
...
func InspectChaincodeDiscrepancies(n *Network, channel string, chaincode Chaincode, checkOrgs []*Organization, peers ...*Peer) {
	sess, err := n.PeerCliAdminSession(peers[0], commands.ChaincodeCheckCommitReadiness{
		ChannelID:           channel,
		Name:                chaincode.Name,
...
func GetLedgerHeight(n *Network, peer *Peer, channel string) int {
	sess, err := n.PeerCliUserSession(peer, "User1", commands.ChannelInfo{
		ChannelID:  channel,
		ClientAuth: n.ClientAuthRequired,
	})
	Expect(err).NotTo(HaveOccurred())
