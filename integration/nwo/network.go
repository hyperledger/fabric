...
// single file to be used by peercli.
...
func (n *Network) CreateChannel(channelName string, o *Orderer, p *Peer, additionalSigners ...any) {
	...
	createChannel := func() int {
		sess, err := n.PeerCliAdminSession(p, commands.ChannelCreate{
...
func (n *Network) signConfigTransaction(channelTxPath string, submittingPeer *Peer, signers ...any) {
	for _, signer := range signers {
		...
		case *Peer:
			sess, err := n.PeerCliAdminSession(signer, commands.SignConfigTx{
				File:       channelTxPath,
				ClientAuth: n.ClientAuthRequired,
			})
			Expect(err).NotTo(HaveOccurred())
...
func (n *Network) JoinChannel(name string, o *Orderer, peers ...*Peer) {
	...
	for _, p := range peers {
		sess, err := n.PeerCliAdminSession(p, commands.ChannelJoin{
...
func (n *Network) JoinChannelBySnapshot(snapshotDir string, peers ...*Peer) {
	...
	for _, p := range peers {
		sess, err := n.PeerCliAdminSession(p, commands.ChannelJoinBySnapshot{
...
func (n *Network) JoinBySnapshotStatus(p *Peer) string {
	sess, err := n.PeerCliAdminSession(p, commands.ChannelJoinBySnapshotStatus{
		ClientAuth: n.ClientAuthRequired,
	})
	Expect(err).NotTo(HaveOccurred())
	Eventually(sess, n.EventuallyTimeout).Should(gexec.Exit(0))
...
func (n *Network) peerCommand(command Command, tlsDir string, env ...string) *exec.Cmd {
	...
	// the correct peer usage, so we count the number of --peerAddresses
	// usages we have, and add the same (concatenated TLS CA certificates file)
	// the same number of times to bypass the peer sanity checks
...
func (n *Network) peerCliCommand(command Command, tlsDir string, env ...string) *exec.Cmd {
	cmd := NewCommand(n.Components.PeerCli(), command)
	cmd.Env = append(cmd.Env, env...)
	...
	// In case we have a peer invoke with multiple certificates, we need to mimic
	// the correct peercli usage, so we count the number of --peerAddresses
	// usages we have, and add the same (concatenated TLS CA certificates file)
	// the same number of times to bypass the peercli sanity checks
	requiredPeerAddresses := flagCount("--peerAddresses", cmd.Args)
...
// PeerCliAdminSession starts a gexec.Session as a peer admin for the provided
// peer command. This is intended to be used by short running peercli commands
// that execute in the context of a peer configuration.
func (n *Network) PeerCliAdminSession(p *Peer, command Command) (*gexec.Session, error) {
	return n.PeerCliUserSession(p, "Admin", command)
}

// PeerCliUserSession starts a gexec.Session as a peer user for the provided peer
// command. This is intended to be used by short running peercli commands that
// execute in the context of a peer configuration.
func (n *Network) PeerCliUserSession(p *Peer, user string, command Command) (*gexec.Session, error) {
	cmd := n.peerCliCommand(
		command,
...
// command. This is intended to be used by short running peer commands that
// execute in the context of a peer configuration.
