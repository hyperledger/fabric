...
import (
	...
	gcomm "github.com/hyperledger/fabric/v3/gossip/comm"
	gossipcommon "github.com/hyperledger/fabric/v3/gossip/common"
	"github.com/hyperledger/fabric/v3/gossip/discovery"
	"github.com/hyperledger/fabric/v3/gossip/election"
	"github.com/hyperledger/fabric/v3/gossip/gossip"
	"github.com/hyperledger/fabric/v3/gossip/gossip/algo"
	"github.com/hyperledger/fabric/v3/gossip/gossip/channel"
	gossipmetrics "github.com/hyperledger/fabric/v3/gossip/metrics"
	"github.com/hyperledger/fabric/v3/gossip/privdata"
	"github.com/hyperledger/fabric/v3/gossip/state"
...
func TestLeaderElectionWithDeliverClient(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		if gossips[i].deliveryService[channelName].(*mockDeliverService).running {
			startsNum++
		}
	}
...
func TestWithStaticDeliverClientLeader(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		require.True(t, gossips[i].deliveryService[channelName].(*mockDeliverService).running, "Block deliverer not started for peer %d", i)
	}
...
func TestWithStaticDeliverClientNotLeader(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		require.NotNil(t, gossips[i].deliveryService[channelName], "Delivery service for channel %s not initiated in peer %d", channelName, i)
		require.False(t, gossips[i].deliveryService[channelName].(*mockDeliverService).running, "Block deliverer should not be started for peer %d", i)
	}
...
func (ds *mockDeliverService) StartDeliverForChannel(chainID string, ledgerInfo blocksprovider.LedgerInfo, finalizer func()) error {
	ds.running = true
	ds.channelID = chainID
	return nil
}

func (ds *mockDeliverService) StopDeliverForChannel() error {
	ds.running = false
	return nil
}
