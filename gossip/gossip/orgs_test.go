...
func extractOrgsFromMsg(msg *proto.GossipMessage, sec api.SecurityAdvisor) []string {
	if protoext.IsAliveMsg(msg) {
		return []string{string(sec.OrgByPeerIdentity(api.PeerIdentityType(msg.GetAliveMsg().GetMembership().GetPkiId())))}
	}
	...
	if msg.GetMemRes() != nil {
		...
		for _, envp := range append(alive, dead...) {
			...
			orgs[string(sec.OrgByPeerIdentity(api.PeerIdentityType(msg.GetAliveMsg().GetMembership().GetPkiId())))] = struct{}{}
		}
	}
