...
func (i *Invocation) InstallChaincode(input *lb.InstallChaincodeArgs) (proto.Message, error) {
	if logger.IsEnabledFor(zapcore.DebugLevel) {
		end := min(len(input.GetChaincodeInstallPackage()), 35)
		...
		packageFingerprint := input.GetChaincodeInstallPackage()[0:end]
		logger.Debugf(
	...
	installedCC, err := i.SCC.Functions.InstallChaincode(input.GetChaincodeInstallPackage())
	if err != nil {
		return nil, err
	}
...
func (i *Invocation) QueryInstalledChaincode(input *lb.QueryInstalledChaincodeArgs) (proto.Message, error) {
	...
	chaincode, err := i.SCC.Functions.QueryInstalledChaincode(input.GetPackageId())
	if err != nil {
		return nil, err
	}
...
func (i *Invocation) GetInstalledChaincodePackage(input *lb.GetInstalledChaincodePackageArgs) (proto.Message, error) {
	...
	pkgBytes, err := i.SCC.Functions.GetInstalledChaincodePackage(input.GetPackageId())
	if err != nil {
		return nil, err
	}
...
func (i *Invocation) ApproveChaincodeDefinitionForMyOrg(input *lb.ApproveChaincodeDefinitionForMyOrgArgs) (proto.Message, error) {
	if err := i.validateInput(input.GetName(), input.GetVersion(), input.GetCollections()); err != nil {
	...
	if input.GetCollections() != nil {
		collectionConfig = input.GetCollections().GetConfig()
	}

	var packageID string
	if input.GetSource() != nil {
		switch source := input.GetSource().GetType().(type) {
	...
	cd := &ChaincodeDefinition{
		...
		EndorsementInfo: &lb.ChaincodeEndorsementInfo{
			...
			EndorsementPlugin: input.GetEndorsementPlugin(),
			InitRequired:      input.GetInitRequired(),
		},
		ValidationInfo: &lb.ChaincodeValidationInfo{
			ValidationPlugin:    input.GetValidationPlugin(),
			ValidationParameter: input.GetValidationParameter(),
		},
...
func (i *Invocation) QueryApprovedChaincodeDefinition(input *lb.QueryApprovedChaincodeDefinitionArgs) (proto.Message, error) {
	...
	return &lb.QueryApprovedChaincodeDefinitionResult{
		...
		Version:             ca.EndorsementInfo.GetVersion(),
		EndorsementPlugin:   ca.EndorsementInfo.GetEndorsementPlugin(),
		ValidationPlugin:    ca.ValidationInfo.GetValidationPlugin(),
		ValidationParameter: ca.ValidationInfo.GetValidationParameter(),
		InitRequired:        ca.EndorsementInfo.GetInitRequired(),
...
func (i *Invocation) CommitChaincodeDefinition(input *lb.CommitChaincodeDefinitionArgs) (proto.Message, error) {
	...
	logger.Infof("Successfully endorsed commit for chaincode name '%s' on channel '%s' with definition {%s}", input.GetName(), i.Stub.GetChannelID(), cd)
...
func (i *Invocation) QueryChaincodeDefinition(input *lb.QueryChaincodeDefinitionArgs) (proto.Message, error) {
	...
	definedChaincode, err := i.SCC.Functions.QueryChaincodeDefinition(input.GetName(), i.Stub)
	if err != nil {
		return nil, err
	}
	...
	if approvals, _, err = i.SCC.Functions.QueryOrgApprovals(input.GetName(), definedChaincode, opaqueStates); err != nil {
		return nil, err
	}
	...
	return &lb.QueryChaincodeDefinitionResult{
		...
		EndorsementPlugin:   definedChaincode.EndorsementInfo.GetEndorsementPlugin(),
		ValidationPlugin:    definedChaincode.ValidationInfo.GetValidationPlugin(),
		ValidationParameter: definedChaincode.ValidationInfo.GetValidationParameter(),
		InitRequired:        definedChaincode.EndorsementInfo.GetInitRequired(),
...
func extractStaticCollectionConfigs(collConfigPkg *pb.CollectionConfigPackage) ([]*pb.StaticCollectionConfig, error) {
	if collConfigPkg == nil || len(collConfigPkg.GetConfig()) == 0 {
		return nil, nil
	}
	collConfigs := make([]*pb.StaticCollectionConfig, len(collConfigPkg.GetConfig()))
	for i, c := range collConfigPkg.GetConfig() {
...
func validateCollectionConfigs(collConfigs []*pb.StaticCollectionConfig, mspMgr msp.MSPManager) error {
	...
	for _, c := range collConfigs {
		if !collectionNameRegExp.MatchString(c.GetName()) {
...
func validateCollectionConfigMemberOrgsPolicy(coll *pb.StaticCollectionConfig, mspMgr msp.MSPManager) error {
	if coll.GetMemberOrgsPolicy() == nil {
		return errors.Errorf("collection member policy is not set for collection '%s'", coll.GetName())
	}
	if coll.GetMemberOrgsPolicy().GetSignaturePolicy() == nil {
		return errors.Errorf("collection member org policy is empty for collection '%s'", coll.GetName())
	}
	...
	if _, err := pp.NewPolicy(coll.GetMemberOrgsPolicy().GetSignaturePolicy()); err != nil {
		return errors.WithMessagef(err, "invalid member org policy for collection '%s'", coll.GetName())
	}

	// make sure that the signature policy is meaningful (only consists of ORs)
	if err := validateSpOrConcat(coll.GetMemberOrgsPolicy().GetSignaturePolicy().GetRule()); err != nil {
		return errors.WithMessagef(err, "collection-name: %s -- error in member org policy", coll.GetName())
	}
	...
	for _, principal := range coll.GetMemberOrgsPolicy().GetSignaturePolicy().GetIdentities() {
		...
		default:
			return errors.Errorf("collection-name: %s -- principal type %v is not supported", coll.GetName(), principal.GetPrincipalClassification())
		}
	}
	return nil
}
...
func validateSpOrConcat(sp *common.SignaturePolicy) error {
	...
	if sp.GetNOutOf().GetN() != 1 {
		return errors.Errorf("signature policy is not an OR concatenation, NOutOf %d", sp.GetNOutOf().GetN())
	}
	// recurse into all sub-rules
	for _, rule := range sp.GetNOutOf().GetRules() {
...
func validateCollConfigsAgainstCommittedDef(
	...
	if committedCollConfPkg == nil || len(committedCollConfPkg.GetConfig()) == 0 {
		return nil
	}
	...
	for _, committedCollConfig := range committedCollConfPkg.GetConfig() {
		...
		newCollection, ok := proposedCollsMap[committedColl.GetName()]
		if !ok {
			return errors.Errorf("existing collection [%s] missing in the proposed collection configuration", committedColl.GetName())
		}

		if newCollection.GetBlockToLive() != committedColl.GetBlockToLive() {
			return errors.Errorf("the BlockToLive in an existing collection [%s] modified. Existing value [%d]", committedColl.GetName(), committedColl.GetBlockToLive())
		}
	}
	return nil
}
