	...
	Describe("Invoke", func() {
		...
		Describe("QueryApprovedChaincodeDefinitions", func() {
			...
			It("passes the arguments to and returns the results from the backing scc function implementation", func() {
				...
				Expect(payload.GetApprovedChaincodeDefinitions()).To(ConsistOf(expectedDefinitions.GetApprovedChaincodeDefinitions()))
			...
			Context("when the underlying QueryApprovedChaincodeDefinitions function implementation returns a definition map with a bad key", func() {
				...
				It("wraps and returns the error", func() {
					res := scc.Invoke(fakeStub)
					Expect(res.GetStatus()).To(Equal(int32(500)))
					Expect(res.GetMessage()).To(Equal("failed to invoke backing implementation of 'QueryApprovedChaincodeDefinitions': could not extract chaincode name and sequence number from private name: badkey: invalid private name format: badkey"))
				})
			})
			...
			Context("when the underlying QueryApprovedChaincodeDefinitions function implementation fails", func() {
				...
				It("wraps and returns the error", func() {
					res := scc.Invoke(fakeStub)
					Expect(res.GetStatus()).To(Equal(int32(500)))
					Expect(res.GetMessage()).To(Equal("failed to invoke backing implementation of 'QueryApprovedChaincodeDefinitions': underlying-error"))
				})
			})
