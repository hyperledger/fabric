...
func (h *Handler) handleMessage(msg *pb.ChaincodeMessage) error {
	chaincodeLogger.Debugf("[%s] Fabric side handling ChaincodeMessage of type: %s in state %s", shorttxid(msg.GetTxid()), msg.GetType(), h.state)
	...
	default:
		return errors.Errorf("handle message: invalid state %s for transaction %s", h.state, msg.GetTxid())
	}
}
...
func (h *Handler) HandleRegister(msg *pb.ChaincodeMessage) {
	chaincodeLogger.Debugf("Received %s in state %s", msg.GetType(), h.state)
...
func (h *Handler) checkMetadataCap(msg *pb.ChaincodeMessage) error {
	ac, exists := h.AppConfig.GetApplicationConfig(msg.GetChannelId())
	if !exists {
		return errors.Errorf("application config does not exist for %s", msg.GetChannelId())
	}
...
func (h *Handler) HandlePutState(msg *pb.ChaincodeMessage, txContext *TransactionContext) (*pb.ChaincodeMessage, error) {
	...
	err := proto.Unmarshal(msg.GetPayload(), putState)
	if err != nil {
		return nil, errors.Wrap(err, "unmarshal failed")
	}

	namespaceID := txContext.NamespaceID
	collection := putState.GetCollection()
	if isCollectionSet(collection) {
		if txContext.IsInitTransaction {
			return nil, errors.New("private data APIs are not allowed in chaincode Init()")
		}
		...
		err = txContext.TXSimulator.SetPrivateData(namespaceID, collection, putState.GetKey(), putState.GetValue())
	} else {
		err = txContext.TXSimulator.SetState(namespaceID, putState.GetKey(), putState.GetValue())
	}
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func (h *Handler) HandlePutStateMetadata(msg *pb.ChaincodeMessage, txContext *TransactionContext) (*pb.ChaincodeMessage, error) {
	...
	err = proto.Unmarshal(msg.GetPayload(), putStateMetadata)
	if err != nil {
		return nil, errors.Wrap(err, "unmarshal failed")
	}

	metadata := make(map[string][]byte)
	metadata[putStateMetadata.GetMetadata().GetMetakey()] = putStateMetadata.GetMetadata().GetValue()

	namespaceID := txContext.NamespaceID
	collection := putStateMetadata.GetCollection()
	if isCollectionSet(collection) {
		if txContext.IsInitTransaction {
			return nil, errors.New("private data APIs are not allowed in chaincode Init()")
		}
		...
		err = txContext.TXSimulator.SetPrivateDataMetadata(namespaceID, collection, putStateMetadata.GetKey(), metadata)
	} else {
		err = txContext.TXSimulator.SetStateMetadata(namespaceID, putStateMetadata.GetKey(), metadata)
	}
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func (h *Handler) HandleDelState(msg *pb.ChaincodeMessage, txContext *TransactionContext) (*pb.ChaincodeMessage, error) {
	...
	err := proto.Unmarshal(msg.GetPayload(), delState)
	if err != nil {
		return nil, errors.Wrap(err, "unmarshal failed")
	}

	namespaceID := txContext.NamespaceID
	collection := delState.GetCollection()
	if isCollectionSet(collection) {
		if txContext.IsInitTransaction {
			return nil, errors.New("private data APIs are not allowed in chaincode Init()")
		}
		...
		err = txContext.TXSimulator.DeletePrivateData(namespaceID, collection, delState.GetKey())
	} else {
		err = txContext.TXSimulator.DeleteState(namespaceID, delState.GetKey())
	}
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func (h *Handler) HandlePurgePrivateData(msg *pb.ChaincodeMessage, txContext *TransactionContext) (*pb.ChaincodeMessage, error) {
	...
	if err := txContext.TXSimulator.PurgePrivateData(namespaceID, collection, delState.GetKey()); err != nil {
		return nil, errors.WithStack(err)
	}
