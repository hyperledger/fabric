...
func testDataKeyEncoding(t *testing.T, _ string, ns string, key string) {
...
// TestQueryOnBadgerDB tests queries on badgerdb.
func TestQueryOnBadgerDB(t *testing.T) {
	env := NewTestVDBEnv(t)
	defer env.Cleanup()
	...
	// As queries are not supported in badgerdb out of the box,
	// call to ExecuteQuery() should return a error message.
	// But actually queries are possible with usage the tool
