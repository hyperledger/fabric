...
// GetStateRangeScanIteratorWithLimit implements method in VersionedDB interface
// startKey is inclusive
// endKey is exclusive
...
func (vdb *VersionedDB) GetStateRangeScanIteratorWithLimit(namespace string, startKey string, endKey string, limit int32) (statedb.QueryResultsIterator, error) {
	logger.Debugf("Entering GetStateRangeScanIteratorWithOptions namespace: %s  startKey: %s  endKey: %s  options: %d", namespace, startKey, endKey, limit)
	internalQueryLimit := vdb.couchInstance.InternalQueryLimit()
	db, err := vdb.getNamespaceDBHandle(namespace)
	if err != nil {
		return nil, err
	}
	return newQueryScanner(namespace, db, "", internalQueryLimit, limit, "", startKey, endKey)
}
...
func (vdb *VersionedDB) ExecuteQuery(namespace, query string) (statedb.ResultsIterator, error) {
	queryResult, err := vdb.ExecuteQueryWithBookmarkAndLimit(namespace, query, "", 0)
	if err != nil {
		return nil, err
	}
...
// ExecuteQueryWithBookmarkAndLimit implements method in VersionedDB interface
func (vdb *VersionedDB) ExecuteQueryWithBookmarkAndLimit(namespace, query, bookmark string, limit int32) (statedb.QueryResultsIterator, error) {
	logger.Debugf("Entering ExecuteQueryWithMetadata  namespace: %s,  query: %s,  metadata: %d", namespace, query, limit)
	...
	return newQueryScanner(namespace, db, queryString, internalQueryLimit, limit, bookmark, "", "")
}
