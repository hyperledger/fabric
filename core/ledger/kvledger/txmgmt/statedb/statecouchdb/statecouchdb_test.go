...
func TestUtilityFunctions(t *testing.T) {
	...
	for _, reservedField := range reservedFields {
		...
		require.Errorf(t, err, "ValidateKey should have thrown an error for a json value %s, as contains one of the reserved fields", testVal)
	}
	...
	for _, reservedField := range reservedFields {
		...
		err = db.ValidateKeyValue("testKey", []byte(testVal))
		require.NoErrorf(t, err, "ValidateKey should not have thrown an error the json value %s since the reserved field was not at the top level", testVal)
	}
