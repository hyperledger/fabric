...
func (scanner *historyScanner) Next() (commonledger.QueryResult, error) {
	for {
		...
		if err != nil {
			logger.Warningf("Some other key [%#v] found in the range while scanning history for key [%#v]. Skipping (decoding error: %s)",
		...
		if err == blkstorage.ErrNotFoundInIndex {
			logger.Warningf("Some other clashing key [%#v] found in the range while scanning history for key [%#v]. Skipping (cannot find block:tx)",
		...
		if queryResult == nil {
			...
			logger.Warningf("Some other key [%#v] found in the range while scanning history for key [%#v]. Skipping (namespace or key not found)",
				historyKey, scanner.key)
			continue
		}
