...
func newKVLedger(ledgerID string, blockStore *ledgerstorage.Store,
	...
	l := &kvLedger{ledgerID, blockStore, txmgmt, historyDB, &sync.RWMutex{}, nil}

	// Retrieves the current commit hash from the blockstore
	var err error
	l.commitHash, err = l.lastPersistedCommitHash()
	if err != nil {
		panic(fmt.Errorf("Error getting current commit hash: %s", err))
	}
...
func (l *kvLedger) CommitWithPvtData(pvtdataAndBlock *ledger.BlockAndPvtData) error {
	...
	updateBatchBytes, err := l.txtmgmt.ValidateAndPrepare(pvtdataAndBlock, true)
	if err != nil {
		return err
	}
