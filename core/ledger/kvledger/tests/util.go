...
func convertFromCollConfigProto(collConfPkg *common.CollectionConfigPackage) []*collConf {
	...
	for _, protoConf := range protoConfArray {
		name := protoConf.GetStaticCollectionConfig().Name
		memberOrgsPolicy := protoConf.GetStaticCollectionConfig().MemberOrgsPolicy
		collConfs = append(collConfs, &collConf{name: name, members: convertFromMemberOrgsPolicy(memberOrgsPolicy)})
	}
	return collConfs
}
...
func setBlockFlagsToValid(block *common.Block) {
	...
	block.Metadata.Metadata[common.BlockMetadataIndex_TRANSACTIONS_FILTER] =
		lutils.NewTxValidationFlags(len(block.Data.Data))
}
