...
func convertFromMemberOrgsPolicy(policy *protopeer.CollectionPolicyConfig) []string {
	...
	ids := policy.GetSignaturePolicy().GetIdentities()
	var members []string
	for _, id := range ids {
		role := &msp.MSPRole{}
		err := proto.Unmarshal(id.GetPrincipal(), role)
		if err == nil {
			...
			members = append(members, role.GetMspIdentifier())
		} else {
			...
			members = append(members, string(id.GetPrincipal()))
		}
	}
	return members
}
...
func convertFromCollConfigProto(collConfPkg *protopeer.CollectionConfigPackage) []*collConf {
	...
	for _, protoConf := range protoConfArray {
		...
		collConfs = append(
			...
			&collConf{
				name:    p.GetName(),
				btl:     p.GetBlockToLive(),
				members: convertFromMemberOrgsPolicy(p.GetMemberOrgsPolicy()),
			},
		)
	}
	return collConfs
...
func setBlockFlagsToValid(block *common.Block) {
	...
	block.Metadata.Metadata[common.BlockMetadataIndex_TRANSACTIONS_FILTER] = txflags.NewWithValues(len(block.GetData().GetData()), protopeer.TxValidationCode_VALID)
}
