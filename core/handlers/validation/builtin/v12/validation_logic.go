	...
	if len(lsccrwset.GetWrites()) == 2 {
		...
		if lsccrwset.GetWrites()[1].GetKey() != key {
			return policyErr(fmt.Errorf("invalid key for the collection of chaincode %s:%s; expected '%s', received '%s'",
				cdRWSet.Name, cdRWSet.Version, key, lsccrwset.GetWrites()[1].GetKey()))
		}
...
func (vscc *Validator) ValidateLSCCInvocation(
	...
	case lscc.UPGRADE, lscc.DEPLOY:
		...
		if cdRWSet.Name != cdsArgs.GetChaincodeSpec().GetChaincodeId().GetName() {
			return policyErr(fmt.Errorf("expected cc name %s, found %s", cdsArgs.GetChaincodeSpec().GetChaincodeId().GetName(), cdRWSet.Name))
		}
		// the chaincode version in the lsccwriteset must match the chaincode version in the deployment spec
		if cdRWSet.Version != cdsArgs.GetChaincodeSpec().GetChaincodeId().GetVersion() {
			return policyErr(fmt.Errorf("expected cc version %s, found %s", cdsArgs.GetChaincodeSpec().GetChaincodeId().GetVersion(), cdRWSet.Version))
		}
		// it must only write to 2 namespaces: LSCC's and the cc that we are deploying/upgrading
		for _, ns := range txRWSet.NsRwSets {
			if ns.NameSpace != "lscc" && ns.NameSpace != cdRWSet.Name && len(ns.KvRwSet.GetWrites()) > 0 {
		...
		case lscc.UPGRADE:
			...
			/**********************************************************/
			if cdLedger.Version == cdsArgs.GetChaincodeSpec().GetChaincodeId().GetVersion() {
