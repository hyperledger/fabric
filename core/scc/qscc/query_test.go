...
func setupTestLedger(t *testing.T, chainid string, path string) (*mocks2.ChaincodeStub, *LedgerQuerier, *peer.Peer, func(), error) {
	...
	if res := lq.Init(&mockStub); res.GetStatus() != shim.OK {
		return nil, lq, peerInstance, cleanup, fmt.Errorf("Init failed for test ledger [%s] with message: %s", chainid, res.GetMessage())
	}
...
func TestQueryGetChainInfo(t *testing.T) {
	...
	require.Equal(t, "Invalid chain ID, fakechainid", res.GetMessage())
}
...
func TestQueryGetTransactionByID(t *testing.T) {
	...
	require.Equal(t, "Failed to get transaction with id 1, error no such transaction ID [1] in index", res.GetMessage())
	...
	require.Equal(t, "Transaction ID must not be nil.", res.GetMessage())
	...
	require.Equal(t, "missing 3rd argument for GetTransactionByID", res.GetMessage())
}
...
func TestQueryGetBlockByNumber(t *testing.T) {
	...
	require.Equal(t, "Failed to get block number 1, error no such block number [1] in index", res.GetMessage())
	...
	require.Equal(t, "Block number must not be nil.", res.GetMessage())
}
...
func TestQueryGetBlockByHash(t *testing.T) {
	...
	require.Equal(t, "Failed to get block hash 0, error no such block hash [30] in index", res.GetMessage())
	...
	require.Equal(t, "Block hash must not be nil.", res.GetMessage())
}
...
func TestQueryGetBlockByTxID(t *testing.T) {
	...
	require.Equal(t, "Failed to get block for txID , error no such transaction ID [] in index", res.GetMessage())
}
...
func TestFailingAccessControl(t *testing.T) {
	...
	res := lq.Invoke(mockStub)
	require.Equal(t, int32(shim.ERROR), res.GetStatus(), "GetChainInfo must fail: %s", res.GetMessage())
	require.Equal(t, "access denied for [GetChainInfo][mytestchainid6]: [Failed access control]", res.GetMessage())
	...
	require.Equal(t, "access denied for [GetBlockByNumber][mytestchainid6]: [Failed access control]", res.GetMessage())
	// assert that the expectations were met
	mockAclProvider.AssertExpectations(t)
	...
	require.Equal(t, "access denied for [GetBlockByHash][mytestchainid6]: [Failed access control]", res.GetMessage())
	// assert that the expectations were met
	mockAclProvider.AssertExpectations(t)
	...
	require.Equal(t, "access denied for [GetBlockByTxID][mytestchainid6]: [Failed access control]", res.GetMessage())
	// assert that the expectations were met
	mockAclProvider.AssertExpectations(t)
	...
	require.Equal(t, "access denied for [GetTransactionByID][mytestchainid6]: [Failed access control]", res.GetMessage())
	// assert that the expectations were met
	mockAclProvider.AssertExpectations(t)
}
...
func TestQueryNonexistentFunction(t *testing.T) {
	...
	require.Equal(t, "Requested function GetBlocks not found.", res.GetMessage())
}
