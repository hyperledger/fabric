		...
		case sn := <-c.snapC:
			...
			if err := c.catchUp(sn); err != nil {
				c.logger.Panicf("Failed to recover from snapshot taken at Term %d and Index %d: %s",
					sn.GetMetadata().Term, sn.GetMetadata().GetIndex(), err)
			}
...
func (c *Chain) isConfig(env *common.Envelope) (bool, error) {
	...
	return h.GetType() == int32(common.HeaderType_CONFIG) || h.GetType() == int32(common.HeaderType_ORDERER_TRANSACTION), nil
}
