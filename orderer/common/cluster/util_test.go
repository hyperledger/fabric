...
func TestConfigFromBlockBadInput(t *testing.T) {
	for _, testCase := range []struct {
		...
		},
		{
			...
			expectedError: "error unmarshalling Envelope: proto: cannot parse invalid wire-format data",
			block:         &common.Block{Data: &common.BlockData{Data: [][]byte{{1, 2, 3}}}},
		},
		{
			name:          "bad genesis block",
			expectedError: "invalid config envelope: proto: cannot parse invalid wire-format data",
			block: &common.Block{
		...
		},
		{
			...
			expectedError: "error unmarshalling ChannelHeader: proto: cannot parse invalid wire-format data",
			block: &common.Block{
