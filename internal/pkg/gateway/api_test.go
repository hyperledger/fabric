...
func TestEndorse(t *testing.T) {
	tests := []testDef{
		...
		{
			name: "extra endorsers with transient data will fail",
			plan: endorsementPlan{
				"g1": {{endorser: localhostMock, height: 3}, {endorser: peer1Mock, height: 4}}, // msp1
				"g2": {{endorser: peer4Mock, height: 5}},                                       // msp3
			},
			transientData: map[string][]byte{"transient-key": []byte("transient-value")},
			errString:     "rpc error: code = FailedPrecondition desc = non-local organizations are required to endorse; retry specifying endorsing organization(s) to protect transient data",
		},
		{
