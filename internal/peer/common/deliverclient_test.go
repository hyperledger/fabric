...
func TestSeekHelper(t *testing.T) {
	t.Run("Standard", func(t *testing.T) {
		...
		require.Equal(t, ab.SeekInfo_BLOCK_UNTIL_READY, seekInfo.GetBehavior())
		require.Equal(t, ab.SeekInfo_STRICT, seekInfo.GetErrorResponse())
	})
	...
	t.Run("BestEffort", func(t *testing.T) {
		...
		_, err := protoutil.UnmarshalEnvelopeOfType(env, cb.HeaderType_DELIVER_SEEK_INFO, seekInfo)
		require.NoError(t, err)
		require.Equal(t, ab.SeekInfo_BEST_EFFORT, seekInfo.GetErrorResponse())
	})
}
