...
func (s *MSPMessageCryptoService) VerifyBlock(chainID common.ChannelID, seqNum uint64, block *pcommon.Block) error {
	...
	if !bytes.Equal(protoutil.BlockDataHash(block.GetData()), block.GetHeader().GetDataHash()) {
	...
	for _, metadataSignature := range metadata.GetSignatures() {
		...
		signatureSet = append(
			...
			&protoutil.SignedData{
				...
				Data:      util.ConcatenateBytes(metadata.GetValue(), metadataSignature.GetSignatureHeader(), protoutil.BlockHeaderBytes(block.GetHeader())),
