...
func fetchCmd(cf *ChannelCmdFactory, isNew bool) *cobra.Command {
	...
	if !isNew {
		fetchCmd.Short = "[DEPRECATED] Fetch a block (use the \"peercli channel fetch\" or \"osnadmin channel fetch\")."
		fetchCmd.Long = "[DEPRECATED] Fetch a specified block from peer or orderer, writing it to a file. Instead of this command, use \"peercli channel fetch\" or \"osnadmin channel fetch\"."
	}
	flagList := []string{
		"channelID",
		"bestEffort",
...
func fetch(cmd *cobra.Command, args []string, cf *ChannelCmdFactory, isNew bool) error {
	...
	if isNew && ordererRequired {
		return errors.Errorf("peercli channel fetch only supports fetching blocks from a peer; to fetch blocks from an orderer, use \"osnadmin channel fetch\"")
	}
	if cf == nil {
