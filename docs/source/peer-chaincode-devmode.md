...
4. Set the `FABRIC_CFG_PATH` environment variable to point to the `sampleconfig` folder:
    ...
    ```
5. Create the `hyperledger` subdirectory in the `/var` directory. This is a location the orderer uses to store blocks as defined in the orderer (`orderer.yaml`) and peer (`core.yaml`). To create the `hyperledger` subdirectory, execute these commands, replacing the question marks with your username:
