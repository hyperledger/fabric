...
Use "cli channel [command] --help" for more information about a command.
```
...
Usage:
  cli channel fetch <newest|oldest|config|(number)> [outputfile] [flags]
...
## cli channel joinbysnapshot
```
Joins the peer to a channel by the specified snapshot

Usage:
  cli channel joinbysnapshot [flags]
...
Usage:
  cli channel joinbysnapshotstatus [flags]
