...
Available Commands:
  create               [DEPRECATED] Create a channel (use the "osnadmin channel join").
  fetch                [DEPRECATED] Fetch a block (use the "peercli channel fetch" or "osnadmin channel fetch").
  ...
  update               [DEPRECATED] Send a configtx update (use the "osnadmin channel update").
...
## peer channel fetch
```
[DEPRECATED] Fetch a specified block from peer or orderer, writing it to a file. Instead of this command, use "peercli channel fetch" or "osnadmin channel fetch".
