...
Label provides the names of the labels that can be attached to a metric. For example, a `channel` label denotes a channel is associated with this metric.
...
The following metrics are currently emitted for consumption byStatsD. The ``%{variable_name}`` nomenclature represents segments thatvary based on context.

For example, %{channel} will be replaced with the name of the channelassociated with the metric.
