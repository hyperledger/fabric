...
### cli channel create examples (deprecated)

Here's an example that uses the `--orderer` global flag on the `cli channel
create` command.
...
* Create a sample channel `mychannel` defined by the configuration transaction
  ...
  ```
  cli channel create -c mychannel -f ./createchannel.tx --orderer orderer.example.com:7050
...
Here's an example of the `cli channel create` command option.
...
* Create a new channel `mychannel` for the network, using the orderer at ip
  ...
  ```
    cli channel create -c mychannel --orderer orderer.example.com:7050 -f ./createchannel.tx -t 30s
...
### cli channel fetch example

Here's some examples of the `cli channel fetch` command.

* Using the `newest` option to retrieve the most recent channel block, and
  store it in   the file `mychannel.block`.

  ```
  cli channel fetch newest mychannel.block -c mychannel --orderer orderer.example.com:7050
...
* Using the `(block number)` option to retrieve a specific block -- in this
  ...
  ```
  cli channel fetch 16  -c mychannel --orderer orderer.example.com:7050
...
### cli channel getinfo example

Here's an example of the `cli channel getinfo` command.

* Get information about the local peer for channel `mychannel`.

  ```
  cli channel getinfo -c mychannel
...
### cli channel join example

Here's an example of the `cli channel join` command.
...
* Join a peer to the channel defined in the genesis block identified by the file
  ...
  previously retrieved by the `cli channel fetch` command.

  ```
  cli channel join -b ./mychannel.genesis.block
...
### cli channel joinbysnapshot example

Here's an example of the `cli channel joinbysnapshot` command.
...
* Join a peer to the channel from a snapshot identified by the directory
  ...
  ```
  cli channel joinbysnapshot --snapshotpath /snapshots/completed/testchannel/1000
  ...
  2020-10-12 11:41:45.444 EDT [channelCmd] joinBySnapshot -> INFO 003 The joinbysnapshot operation is in progress. Use "cli channel joinbysnapshotstatus" to check the status.

  ```

  You can see that the peer has successfully made a request to join the channel from the specified snapshot.
  When a `joinbysnapshot` operation is in progress, you cannot run another `cli channel join`
  or `cli channel joinbysnapshot` simultaneously. To know whether or not a joinbysnapshot operation is in progress,
  you can call the `cli channel joinbysnapshotstatus` command.


### cli channel joinbysnapshotstatus example

Here are some examples of the `cli channel joinbysnapshotstatus` command.
...
* Query if joinbysnapshot is in progress for any channel. If yes,
  ...
  ```
  cli channel joinbysnapshotstatus
...
### cli channel list example

  Here's an example of the `cli channel list` command.
...
### cli channel signconfigtx example

Here's an example of the `cli channel signconfigtx` command.
...
* Sign the `channel update` transaction defined in the file
  ...
  cli channel signconfigtx -f updatechannel.tx
...
### cli channel update example

Here's an example of the `cli channel update` command.
...
* Update the channel `mychannel` using the configuration transaction defined in
  ...
  ```
  cli channel update -c mychannel -f ./updatechannel.tx -o orderer.example.com:7050
