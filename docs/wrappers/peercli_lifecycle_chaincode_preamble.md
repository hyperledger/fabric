# peercli lifecycle chaincode

The `peercli lifecycle chaincode` subcommand allows administrators to use the
...
The `peercli lifecycle chaincode` command has the following subcommands:
...
Each peercli lifecycle chaincode subcommand is described together with its options in its own
