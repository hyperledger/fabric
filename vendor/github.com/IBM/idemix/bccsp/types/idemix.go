...
type SignatureScheme interface {
	...
	// msg: message signed;
	// nym: if non-nil, the Nym public key that the signature is expected to be bound to; the signature
	// is rejected if the Nym it embeds does not match, preventing an adversary from pairing a valid
	// signature with an unrelated, attacker-chosen Nym public key. If nil, this check is skipped.
	// attributes: as described above;
	// rhIndex: revocation handle index relative to attributes;
	...
	Verify(ipk IssuerPublicKey, signature, msg []byte, nym *math.G1, attributes []IdemixAttribute, rhIndex, eidIndex, skIndex int,
