...
type SignatureScheme interface {
	...
	Sign(cred []byte, sk *math.Zr, Nym *math.G1, RNym *math.Zr, ipk IssuerPublicKey, attributes []bccsp.IdemixAttribute, msg []byte, rhIndex, eidIndex int, cri []byte, sigType bccsp.SignatureType, metadata *bccsp.IdemixSignerMetadata) ([]byte, *bccsp.IdemixSignerMetadata, error)
	...
	AuditNymEid(ipk IssuerPublicKey, eidIndex int, signature []byte, enrollmentID string, RNymEid *math.Zr, verType bccsp.AuditVerificationType) error
}
