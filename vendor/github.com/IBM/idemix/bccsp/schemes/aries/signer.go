...
func (s *Signer) getPoKOfSignature(
	...
	if sigtype == types.Smartcard || sigtype == types.SmartcardNoNyms {
		...
		// `NewPoKOfSignatureExt` as their as, since their correctiness has already
...
func (s *Signer) packageProof(
	...
	if nymEid != nil {
		...
		sig.NymEidIdx = int32(nymEid.index) // #nosec G115 -- index is from internal attribute list, always within int32 range
	}

	if rhNym != nil {
		sig.NymRh = rhNym.comm.Bytes()
		sig.NymRhProof = proofRhNym.ToBytes()
		sig.NymRhIdx = int32(rhNym.index) // #nosec G115 -- index is from internal attribute list, always within int32 range
	}
...
func (s *Signer) getAttributeCommitment(
	...
	if auditData != nil {
		if !attr.Equals(auditData.Attr) {
			return nil, errors.New("attribute supplied in metadata differs from signed")
		}
		...
		if !auditData.Nym.Equals(Nym) {
			return nil, errors.New("nym supplied in metadata cannot be recomputed")
		}
	} else {
...
func (s *Signer) Sign(
	...
	if (sigType == types.EidNym || sigType == types.Smartcard) &&
		...
		return nil, nil, errors.New("cannot create idemix signature: disclosure of enrollment ID requested for EidNym signature")
	}

	if (sigType == types.EidNymRhNym || sigType == types.Smartcard) &&
		(attributes[eidIndex].Type != types.IdemixHiddenAttribute ||
			attributes[rhIndex].Type != types.IdemixHiddenAttribute) {
		return nil, nil, errors.New("cannot create idemix signature: disclosure of enrollment ID or RH requested for EidNymRhNym signature")
	}
	...
	if cri.RevocationAlg != int32(types.AlgNoRevocation) {
		return nil, nil, errors.New("unsupported revocation algorithm")
	}
...
func (s *Signer) Verify(
	...
	if sig.NonRevocationProof == nil {
		return errors.New("signature has no non-revocation proof")
	}

	if sig.NonRevocationProof.RevocationAlg != int32(types.AlgNoRevocation) {
		return errors.New("unsupported revocation algorithm")
	}
	...
	if verifyRHNym && meta != nil {
		...
		if len(meta.RhNym) != 0 {
			...
			if err != nil {
				return errors.New("signature invalid: rh nym validation failed, failed to unmarshal meta rh nym")
			}
			if !RhNym_.Equals(RhNym) {
				return errors.New("signature invalid: rh nym validation failed, signature rh nym does not match metadata")
			}
		}
	}
