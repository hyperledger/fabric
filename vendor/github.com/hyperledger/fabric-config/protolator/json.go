...
func mapToProto(tree map[string]interface{}, msg proto.Message) error {
	...
	return protojson.Unmarshal(jsonOut, msg)
}
...
func protoFields(msg proto.Message, uMsg proto.Message) ([]protoField, error) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		fieldValue := mVal.FieldByName(f.Name)
		fieldTypeStruct, ok := mVal.Type().FieldByName(f.Name)
		if !ok {
		...
		for j, factory := range fieldFactories {
			...
			iResult[j] = append(iResult[j], field)
			break
		}
	}
