...
func TestPeersForEndorsement(t *testing.T) {
	...
	cc := "chaincode"
	mf := &metadataFetcher{}
	mf.On("ChaincodeMetadata").Return(&chaincode.InstantiatedChaincode{
		Name: cc, Version: "1.0",
	}).Times(4)
	g := &gossipMock{}
...
func (mf *metadataFetcher) ChaincodeMetadata(channel string, cc string) *chaincode.InstantiatedChaincode {
	arg := mf.Called().Get(0)
	if arg == nil {
		return nil
	}
	return arg.(*chaincode.InstantiatedChaincode)
}
